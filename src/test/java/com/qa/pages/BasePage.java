...
public class BasePage {
    ...
    public boolean isVisible(WebElement e) {
        try {
            WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(TestUtils.WAIT));
            wait.until(ExpectedConditions.visibilityOf(e));
            return true; // Element görünür oldu
        } catch (TimeoutException ex) {
            System.out.println("Element is not visible within the wait time. Continuing test.");
            return false; // Element görünür değil
        }
    }
    public void waitForElement(WebElement element) {
        try {
            WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(40));
            wait.until(ExpectedConditions.visibilityOf(element)); // pin1 elementi görünür olana kadar bekle
            System.out.println("PIN screen is displayed, ready for input.");
        } catch (TimeoutException e) {
            System.out.println("PIN screen did not appear within the timeout.");
        }
    }
